...
public class AdminAnalyticsActivity extends AppCompatActivity {
            ...
            .addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot snapshot) {
                    ...
                    for (DataSnapshot machine : snapshot.getChildren()) {
                        ...
                        // Filter by admin's building only
                        String building = machine.child("buildingCode")
                                .getValue(String.class);
                        if (adminBuilding != null
                                && !adminBuilding.isEmpty()
                                && (building == null
                                || !building.equals(adminBuilding))) {
                            continue;
                        }

                        // Use machineName for labels
                    ...
                    tvTotalRevenue.setText(String.format(
                            Locale.US, "$%.2f CAD", totalRevenue));
                    tvTotalCycles.setText(String.valueOf(totalCycles));
                    tvPeakHour.setText(peakHour + ":00 - "
                            + (peakHour + 1) + ":00");
    ...
    private void updatePeakHoursChart(Map<Integer, Integer> cyclesPerHour) {
        List<BarEntry> entries = new ArrayList<>();
        for (int h = 0; h < 24; h++) {
            entries.add(new BarEntry(h,
                    cyclesPerHour.getOrDefault(h, 0)));
        }
